...
import { sendMessageToStory } from "@/api/deepseekApi";
...
const STORAGE_KEY = "story_chat_log";
...
async function sendMessage() {
  ...
  try {
    ...
    const botReply = await sendMessageToStory(userText, filteredLog);
    chatLog.value.push({
      id: Date.now() + 1,
      role: "bot",
      text: botReply,
  ...
  return [
    {
      id: Date.now(),
      role: "bot",
      text: `2039年某个夜晚，东京郊区的废弃工厂突然响起枪声与爆炸声。我（樱满集）手握相机，只为完成音乐短片的拍摄任务，却被突如其来的混乱震撼——一名粉色短发少女跌入水渠，她身边散落着一个透明药瓶，她果断躲入暗处。短暂的平静后，机械化重装部队（Endlave）开始搜捕，她又一次被逼到绝路。

我屏住呼吸，镜头对准她的背影，却不知接下来该做什么——我的世界，就在这一瞬间被改写。

下一步建议：

冲入废墟，试图接近女孩并询问她发生了什么。

保持掩护，用相机拍下周围部队动向，以便分析对方行动。

撤回安全地点，拨打电话联系可能的支援——警察、媒体或民间组织。`,
    },
  ];
}
